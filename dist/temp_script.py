...
def convert_table_to_json(table_name, table_data, json_locale_data):
  ...
  for row in rows:
    ...
    json_table_data[row[0]] = {**row_data, **json_data, **ref_data}

    for i in column_indices['locales']:
      json_locale_data[row[i-3]] = {'text': row[i], 'column': column_names[i], 'table': table_name}

    json_path = os.path.join(wb_result_path_dir, f'{table_name}.json')
...
def parse_wb(wb_path):
  ...
  json_locale_data = {}
  json_locale_path = os.path.join(wb_result_path_dir, 'locale.json')
